...
?>
		...
		<h1><?php print _t("Suchergebnisse für \"%1\"", caUcFirstUTF8Safe($this->getVar('searchForDisplay'))); ?></h1>
<?php
		// 
...
<?php
		...
		print "<H1>"._t("Ihre Suche nach \"%1\" ergab keine Ergebnisse", caUcFirstUTF8Safe($this->getVar('search')))."</H1>";
		...
		if (sizeof($va_suggestions = $o_search->suggest($this->getVar('search'), array('returnAsLink' => true, 'request' => $this->request)))) {
			if (sizeof($va_suggestions) > 1) {
				print "<p>"._t("Meinen Sie: %1?", join(', ', $va_suggestions))."</p>";
			} else {
				print "<p>"._t("Meinen Sie %1?", join(', ', $va_suggestions))."</p>";
			}
		}
